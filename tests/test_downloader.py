...
risk_reverse_response = {
    ...
    "data":
        [
            {"symbol":"BTC_USDT","currency":"USDT","available":25236190.842658725020122405,"timestamp":1703750327370},
            {"symbol":"ETH_USDT","currency":"USDT","available":10683529.154410919248666888,"timestamp":1703750327370}
        ]
}
...
def test_validate_symbol():
    ...
    with requests_mock.Mocker() as m:
        m.get("https://contract.mexc.com/api/v1/contract/risk_reverse", json=risk_reverse_response)
...
def test_make_url_with_invalid_symbol():
    ...
    with patch.object(downloader, 'validate_symbol', side_effect=InvalidSymbolFormatError("Invalid symbol: INVALID_SYMBOL")), pytest.raises(InvalidSymbolFormatError):
...
def test_download_all():
    ...
    with patch.object(downloader, 'get_all_symbols_futures', return_value=test_symbols), \
         patch.object(downloader, 'download') as mock_download, \
...
def test_execute_download():
    ...
    with patch("requests.get") as mock_get, \
         patch.object(downloader, '_make_url', return_value=expected_url), \
